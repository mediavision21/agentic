...
def load_templates():
	...
	for path in glob.glob(os.path.join(TEMPLATE_DIR, "**", "*.yaml"), recursive=True):
		...
		templates[rel] = data
	_cache = templates
	print(f"[template_router] loaded {len(templates)} templates")
	return templates
...
async def _resolve_filters(prompt, placeholders, choices_map):
	...
	try:
		text = await llm.complete_text({
			"system": FILTER_RESOLVE_PROMPT,
			"messages": messages,
			"model": "haiku",
			"max_tokens": 300,
			"label": "haiku-filter-resolve",
		})
		debug["response"] = text
...
def _build_filter_defaults_from_intent(intent, placeholders):
	# map resolved intent slots to template filter placeholder values
	resolved = {}
...
async def run_matched_template(options):
	...
	from plot import generate_summary
	try:
		summary = await generate_summary(prompt, data["columns"], data["rows"])
